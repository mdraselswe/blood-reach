...
export function MobileNav({
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'fixed inset-y-0 right-0 z-50 flex w-72 max-w-[80vw] flex-col gap-6 border-l border-slate-100 bg-white p-6 shadow-xl transition-transform md:hidden',
          open ? 'translate-x-0' : 'translate-x-full',
        )}
      >
        <button
          type="button"
          className="self-end rounded-full border border-slate-200 p-2 text-slate-600"
          ...
          {flattenedItems.map((item) => {
            ...
            return (
              <Link
                ...
                href={item.href}
                onClick={() => setOpen(false)}
                className="rounded-full bg-slate-100 px-4 py-3 hover:bg-primary-50 hover:text-primary-600"
              >
                {item.label}
              </Link>
            );
          ...
          {isAuthenticated ? (
            <>
              <Link
                href="/dashboard"
                onClick={() => setOpen(false)}
                className="inline-flex items-center justify-center rounded-full border border-primary/30 px-4 py-3 font-semibold text-primary-600 hover:bg-primary-50"
              >
                ড্যাশবোর্ড
              </Link>
              {isAdmin ? (
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-full bg-primary px-4 py-3 font-semibold text-white shadow-soft hover:bg-primary-600 disabled:opacity-60"
                disabled={loading}
              >
                সাইন আউট
              </button>
            ...
            <>
              <Link
                href="/login"
                onClick={() => setOpen(false)}
                className="inline-flex items-center justify-center rounded-full bg-primary px-4 py-3 font-semibold text-white shadow-soft hover:bg-primary-600"
              >
                লগইন করুন
              </Link>
              <Link
